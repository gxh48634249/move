...
export default {
  ...
  effects: {
    *queryMy({ payload }, { call, put }) {
      ...
      const temp = JSON.parse(respons.results);
      console.log(temp,'123123')
      yield put ({
        type: 'add',
        payload:temp,
      })
  ...
  reducers: {
    add (state, { payload }) {
      console.log(state.myData,'7788')
      const temp = state.myData;
      temp.title = payload.title;
      temp.avatar = payload.avatar;
      temp.description = payload.description;
      temp.content = payload.content;
      temp.message = state.message;
      temp.type = state.type;
      console.log(temp,'gxh');
      state.tuijian.splice(0,1,temp);
      return {
        ...state
      }
    },
